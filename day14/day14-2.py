...
def iterbits(val):
    '''
        >>> [x for x in iterbits(14)]
        [0, 0, 0, 0, 1, 1, 1, 0]
        >>> [x for x in iterbits(255)]
        [1, 1, 1, 1, 1, 1, 1, 1]
    '''
    for x in xrange(8):
        yield 1 if val & 0x80 else 0
        val = val << 1

def iterhash(iterable):
    return itertools.chain(*(iterbits(x) for x in iterable))
...
regions={}
def merge(x, y, region):
    try:
       match = regions[(x, y)]
       match.members.update(region.members)
       for p in match.members:
           regions[p.pos] = match
       return match
    except KeyError:
       return region
...
if __name__ == '__main__':
    ...
    if len(sys.argv) <= 1:
        ...
        #for x in (iterhash(i) for i in hashmatrix(sys.argv[1])):
        #    print "".join(str(b) for b in x)
        for e, b in enumerate(itertools.chain(*(iterhash(i) for i in hashmatrix(sys.argv[1])))):
            x=e % 128
            y=int(e / 128)
            if b:
               match = Region(x,y)
               regions[match.pos] = match
               match = merge(x-1,y, match)
               match = merge(x,y-1, match)
