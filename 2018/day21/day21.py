...
ival = int(sys.argv[2]) if len(sys.argv) > 2 else 0
computer.setreg(0, ival)
repeat=set()
while computer.ip >= 0 and computer.ip < len(memory):
   before = repr(computer)
   ip = computer.ip
   if ip == 28:
        if computer.registers[2] in repeat:
            print "REPEAT %d" % ( computer.registers[2],)
        #print "%20d %24s" % (computer.registers[2], binary(computer.registers[2]))
        repeat.add(computer.registers[2])
   if ip in (26, 28, 6, 8):
