...
def reindeergame(armydata, imboost=0):
    immune = Army()
    infection = Army()

    curarmy = immune
    for line in armydata:
    ...
    print "Immune"
    print '\n'.join(repr(g) for g in immune.groups)
    print "Infect"
    print '\n'.join(repr(g) for g in infection.groups)
    ...
    while True:
        ...
        if immune.size == 0 or infection.size == 0 or (immune.size, infection.size) == last:
            break

        last = (immune.size, infection.size)
    ...
    return (immune.size, infection.size)
...
with open(sys.argv[1]) as f:
    armydata = f.readlines()

#reindeergame(armydata, 1570)
#sys.exit(1)
lastScore, lastMid = 0, 0
l=1
r=32000
while r >= l:
   mid = l + (r - l)/2
   immune, infect = reindeergame(armydata, mid)
   if infect == 0:
       lastMid, lastScore = mid, immune
       r = mid - 1
   else:
       l = mid + 1

print "Boost = %d, Immune Units %d" % (lastMid, lastScore)
